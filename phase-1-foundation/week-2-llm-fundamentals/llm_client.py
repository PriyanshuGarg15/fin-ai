...
class UsageAccumulator:
    ...
    def summary(self)->dict:
        return {
            ...
            "average_cost_per_call": round(self.total_cost_usd / max(self.call_count,1) ,6)
        }
...
class LLMClient:
    def __init__(
        ...
        self._groq_client= AsyncOpenAI(api_key= os.getenv("GROQ_API_KEY"), base_url="https://api.groq.com/openai/v1")
    ...
    async def complete(
        ...
        try:
            response= await self._call_provider(self.primary, prompt, system, model, max_tokens, temperature)
            ...
            if self.fallback:
                logger.warning(f"Primary provider {self.primary} failed: {primary_error}. Falling back to {self.fallback}")
                response = await self._call_provider(self.fallback, prompt, system, None, max_tokens, temperature)
                self.usage.record(response)
                return response
            raise

    async def _call_provider( self, provider: LLMProvider, prompt: str, system: str, model: Optional[str], max_tokens: int, temperature: float)-> LLMResponse:
        ...
        if provider == LLMProvider.GROQ:
            ...
            return LLMResponse(
                ...
                cost_usd=calculate_cost(m, response.usage.prompt_tokens, response.usage.completion_tokens),
            ...
            return LLMResponse(
                ...
                latency_ms=latency_ms,
                cost_usd=calculate_cost(m, response.usage_metadata.prompt_token_count, response.usage_metadata.candidates_token_count),
...
async def demo():
    ...
    for prompt in prompts:
        ...
        print(f"[{response.provider.value}] {response.latency_ms}ms | ${response.cost_usd} | {response.content[:100]}...")
