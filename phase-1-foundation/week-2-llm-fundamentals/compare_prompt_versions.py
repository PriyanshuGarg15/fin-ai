...
async def evaluate_applicant(version_name: str, applicant_data: dict) -> dict:
    ...
    response = await client.chat.completions.create(
        ...
        messages=[{"role": "system", "content": prompt_obj.system_prompt},{"role": "user", "content": user_message}],
...
async def main():
    ...
    print(f"Evaluating Applicant: {tricky_applicant['applicant_id']}")
    print(f"Profile: Self-Employed, Income: 100k, EMIs: 45k\n")
    ...
    print(f"{'Eligible?':<20} | {str(v1_result.get('is_eligible')):<25} | {str(v2_result.get('is_eligible')):<25}")
    print(f"{'Calculated FOIR':<20} | {str(v1_result.get('calculated_foir_percentage')) + '%':<25} | {str(v2_result.get('calculated_foir_percentage')) + '%':<25}")
