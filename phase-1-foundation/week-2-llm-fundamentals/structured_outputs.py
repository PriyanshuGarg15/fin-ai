...
structured_client2= instructor.from_provider("groq/llama-3.1-8b-instant", async_client=True)

# --- GEMINI CLIENT INITIALIZATION ---
# Wrap the new Google GenAI SDK with Instructor
structured_client3 = instructor.from_provider("google/gemini-2.5-flash", async_client=True)
...
class LoanEligibilityAssessment(BaseModel):
    ...
    applicant_id: str
    reasoning: str = Field(..., description="First, calculate the FOIR percentage. Then check if the credit score meets the minimum. Explain the logic before deciding eligibility.")
    is_eligible: bool
    ...
    def amount_must_be_positive_if_eligible(cls, v, info):
        ...
        if info.data.get('is_eligible') and v == 0:
            raise ValueError("Eligible applicants must have a non-zero recommended amount")
        return v
...
async def main():
    ...
    for applicant in test_cases:
        ...
        print(f"Eligible: {result.is_eligible} | Risk: {result.risk_level} | Confidence: {result.confidence_score}")
        print(f"Recommended: INR {result.recommended_loan_amount:,.0f} for {result.recommended_tenure_months} months")
